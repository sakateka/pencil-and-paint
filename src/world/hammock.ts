 ...
 *
 * Baked into the world empty and drawn live when somebody is in it — the trees
 * and the ropes never move, and the person and the sag do. So this is the empty
 * one; `entities/rest.ts` draws the rest.
 *
...
export function traceHammock(
  ...
  for (let i = 0; i <= 22; i++) {
    ...
    const jx = wobble ? rr(-wobble, wobble) : 0;
    const jy = wobble ? rr(-wobble, wobble) : 0;
...
function ropes(
  ...
  for (const side of [0, 1]) {
    ...
    for (const u of side ? [0.94, 1, 1.06] : [-0.06, 0, 0.06]) {
      const p = hammockPoint(x, y, Math.min(1, Math.max(0, u)), sag);
      ctx.beginPath();
      ctx.moveTo(tie.x + rr(-wobble, wobble), tie.y);
      ctx.lineTo(p.x + (u - (side ? 1 : 0)) * 40, p.y + rr(-wobble, wobble));
      ctx.stroke();
    }
  }
}
...
export function drawHammockCloth(
  ...
  isolate(ctx, () => {
    ...
    ctx.lineWidth = 1.15;
    traceHammock(ctx, x, y, sag, 0.7);
    ctx.stroke();
    ctx.globalAlpha = 0.4;
    traceHammock(ctx, x, y, sag + 13, 0.7);
    ctx.stroke();
    ...
    for (let i = 1; i < 9; i++) {
      const p = hammockPoint(x, y, i / 9, sag);
      ctx.beginPath();
      ctx.moveTo(p.x + rr(-0.6, 0.6), p.y - 1);
      ctx.lineTo(p.x + rr(-0.6, 0.6), p.y + 14);
      ctx.stroke();
    }
  });
}
...
export function makeHammock(x: number, y: number): Scenery {
  return {
    ...
    draw(ctx, medium) {
      // A hammock's shadow is the cloth's, not the trees'.
      groundShadow(ctx, x, y - 4, HAMMOCK_SPAN * 0.42, 13, medium);
      drawHammockCloth(ctx, x, y, EMPTY_SAG, medium);
